...
export const create = async (req, res) => {
  ...
  try{
    //make sure title not already created
    ...
    console.log(err);
    return res.status(400).send('Failed to create course. Please try again or contact support')
...
export const readSingleCourse=async(req,res)=>{
  try{
    ...
    console.log("SOMETHING WENT WRONG READING SINGLE COURSE");
...
export const freeEnrollment=async(req,res)=>{
  try{
    ...
    res.json({
      message:'Congratulations you are enrolled successfully ',
      course:result
    })
...
export const stripeSuccess=async(req,res)=>{
  try{
    console.log('Stripe Success 1');

    const course=await Course.findById(req.params.courseId).exec();
    const user=await User.findById(req.user._id).exec();
    console.log('Stripe Success 2');

    if(!user.stripeSession.id){
      res.json({success:false});
      
    }else{
      console.log('Stripe Success 3');
    const session=await stripe.checkout.sessions.retrieve(user.stripeSession.id);
    console.log('Stripe Success 4');
    ...
    console.log("STRIPE SUCCESS FAILLLLEDDDDD",err);
    res.json({success:false});
  }
}
